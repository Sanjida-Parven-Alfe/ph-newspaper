...
const FeaturedNews = ({ sliderNews, subNews }) => {
  ...
  return (
    <section className="relative mb-24 md:mb-16">
      ...
      <Swiper
        ...
        className="mySwiper rounded-2xl overflow-hidden shadow-lg h-[500px] md:h-[600px]"
      >
        {sliderNews.map((news) => (
          <SwiperSlide key={news._id} className="relative w-full h-full group">
            ...
            <div className="absolute bottom-16 md:bottom-24 left-0 w-full p-6 md:p-12 z-10">
              <span className="bg-red-600 text-white text-xs font-bold px-3 py-1 uppercase tracking-wider mb-4 inline-block rounded">
                {news.category}
              </span>
              
              <h1 className="text-3xl md:text-5xl font-extrabold text-white leading-tight mb-4 md:w-3/4 drop-shadow-md">
                <Link href={`/news/${news.category}/${news._id}`} className="hover:text-red-400 transition-colors">
                  {news.title}
                </Link>
              </h1>

              <div className="flex items-center gap-3 text-gray-300 text-sm font-medium">
                <span>{news.author || "PH Reporter"}</span>
      ...
      {subNews && subNews.length > 0 && (
        <div className="relative z-20 -mt-12 mx-4 md:mx-auto md:w-[95%] lg:w-[90%]">
          <div className="bg-base-100 rounded-xl shadow-2xl p-6 grid grid-cols-1 md:grid-cols-2 gap-6 border-t-4 border-red-600 dark:border-red-500">
            {subNews.map((news) => (
              <div key={news._id} className="flex items-center gap-4 group cursor-pointer">
                ...
                <div>
                  ...
                  <h3 className="font-bold text-base-content leading-snug group-hover:text-red-600 transition-colors line-clamp-2">
