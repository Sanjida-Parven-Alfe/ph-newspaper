...
const Navbar = () => {
  ...
  const isParentActive = () => {
    const featuresPaths = ["/news/Tech News", "/news/Startups", "/news/AI & Future", "/news/Web Dev", "/news/Career"];
  ...
  const navLinks = (
    <>
      <li><Link href="/" className={`px-2 py-1 ${pathname === "/" ? " border-b-2 border-blue-600" : "hover:text-blue-600"}`}>Home</Link></li>
      <li><Link href="/news" className={`px-2 py-1 ${pathname === "/news" ? " border-b-2 border-blue-600" : "hover:text-blue-600"}`}>All News</Link></li>
      
      <li>
        <div className="dropdown dropdown-hover px-0">
          <div tabIndex={0} role="button" className={`px-2 py-1 flex items-center gap-1 ${isParentActive()}`}>
            Categories
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
          </div>
          <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
            <li><Link href="/news/Tech News" className={isActive("/news/Tech News")} onClick={closeDropdown}>Tech News</Link></li>
            <li><Link href="/news/Startups" className={isActive("/news/Startups")} onClick={closeDropdown}>Startups</Link></li>
            <li><Link href="/news/AI & Future" className={isActive("/news/AI & Future")} onClick={closeDropdown}>AI & Future</Link></li>
            <li><Link href="/news/Web Dev" className={isActive("/news/Web Dev")} onClick={closeDropdown}>Web Dev</Link></li>
            <li><Link href="/news/Career" className={isActive("/news/Career")} onClick={closeDropdown}>Career</Link></li>
          </ul>
        </div>
      </li>

      <li><Link href="/saradesh" className={`px-2 py-1 font-bold ${pathname === "/saradesh" ? "text-red-600 border-b-2 border-red-600" : "text-red-500 hover:text-red-700"}`}>Sara Desh</Link></li>
    </>
  );
  ...
  return (
    <div className="bg-base-100 border-b border-base-300 sticky top-0 z-[5000] shadow-sm">
